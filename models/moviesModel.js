...
import mongodb from '../managers/mongomanager.js'
...
class MoviesModel {
    async getMovies() {
        ...
        const movies = await mongodb.get_movies('practica');
    ...
    async getMovieById(id) {
        ...
        const _movie = await mongodb.getMovieById('practica',id)
    ...
    async removeMovie(id) {
        ...
        const index = await mongodb.removeMovie('practica',id)

        // const index = movie.removeMovie(id);
        // if (index != -1) { actor.removeActors(id) }
        // return index;
        return ((index.acknowledged == false)? -1: 1);
    }
    ...
    async getMovieBy(elem) {
        ...
        const _movies = await mongodb.get_movies('practica',elem,{});
    ...
    async createMovie(req) {
        ...
        //insertOne
        await mongodb.addMovie('practica',req);
    ...
    async updateMovie(req) {
        ...
        await mongodb.updateMovie('practica',req.id,req);
    ...
    async getMoviesFromActor(req) {
        ...
        const _movies = await mongodb.get_movies('practica',{"actors":{$in:[req.value]}},{});
    ...
    async addActors(req) {
        ...
        await mongodb.updateOne('practica',{id:parseInt(req.id)},{$push:{"actors":req.value}});
    }
